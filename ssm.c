...
static void ssm_parse_response(sesame * ssm, uint8_t cmd_it_code) {
    ...
    case SSM_ITEM_CODE_HISTORY:
        ...
        if (ssm->c_offset == 0) { //履歴の不完全な取得を避けるためのループ読み取り
            send_read_history_cmd_to_ssm(ssm);
            return;
        }
        memcpy((void *) &(ssm->history), ssm->b_buf, 49); //https://document.candyhouse.co/demo/4_history-ja に従って 49 bytes
        ssm->history.tag[ssm->history.tag_lentgh] = '\0';
        ESP_LOGI(TAG, "id=%ld type=%d timestamp=%ld tag=%s", ssm->history.id, ssm->history.type, ssm->history.ts, ssm->history.tag);
        break;
    default:
        break;
    }
